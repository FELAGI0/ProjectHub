...
.PHONY: up down logs test lint format typecheck alembic-check check migrate revision clean help
.DEFAULT_GOAL := help

PYTHON := .venv\Scripts\python.exe

help:  ## Показать все цели
	@$(PYTHON) -c "print('up          Поднять docker compose'); print('down        Остановить docker compose'); print('logs        Показать логи docker compose'); print('test        Запустить тесты'); print('lint        Проверить код через Ruff'); print('format      Отформатировать код и исправить Ruff'); print('typecheck   Проверить типы через mypy'); print('alembic-check Проверить миграции Alembic'); print('check       Запустить все проверки'); print('migrate     Применить миграции'); print('revision    Создать миграцию: make revision msg=описание'); print('clean       Удалить кэши')"
